!=============================================================================
! @file   test_assemble_2d_mpi.f90
! @brief  MPI test for assembling a 2D mesh system (triangular elements).
!
! @details
!   Steps:
!    1) MPI init
!    2) Rank 0 reads the 2D mesh files
...
program test_assemble_2d_mpi
    ...
    if (rank == 0_ip) then
        print *, "=== MPI 2D Assembly Test: Reading mesh on Rank 0 ==="
    end if

    !---------------------------------------------------------------------
    ! 2) Rank 0 reads the 2D mesh from test files
    !---------------------------------------------------------------------
    if (rank == 0_ip) then
    ...
    !---------------------------------------------------------------------
    ! 5) Broadcast the coords, boundary arrays
    !---------------------------------------------------------------------
    ncoords   = mesh%npoin * mesh%ndim   ! Total number of coordinates
    nboundary = mesh%nboun               ! Total number of boundary entries
    ...
    if (rank == 0_ip) then
        print *, "=== 2D Assembly in MPI (Rank 0 final output) ==="
